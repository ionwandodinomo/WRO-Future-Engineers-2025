...
from helper import findContours, preprocess_frame
import numpy as np
...
while True:
    ...
    s = cv.add(s, 20)  # You can adjust this value (e.g., 50-150) for intensity

    # Merge back
    hsv = cv.merge([h, s, v])

    # Convert back to BGR
    frame_hsv = cv.cvtColor(hsv, cv.COLOR_HSV2BGR)
    frame_LAB = cv.cvtColor(frame_hsv, cv.COLOR_BGR2Lab)

    #frame_LAB = cv.bilateralFilter(frame_LAB, d=3, sigmaColor=50, sigmaSpace=50)
    frame_LAB = cv.GaussianBlur(frame, (7, 7), 0)
    ...
    frame_LAB = preprocess_frame(frame)
    
    contours, d_frame = findContours(frame_LAB,[(low_L, low_A, low_B),(high_L, high_A, high_B)],test=True)
