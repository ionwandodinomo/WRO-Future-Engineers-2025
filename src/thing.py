...
#         print("size:", size, "min size", min_size)
        ...
        dynamic_gain2 = PILLAR_PG2 + (1 - normalized_y) * 0.0005
...
#             GREEN_TARGET = 530
        ...
        if size<1200:
            angle = int((-MAX_TURN_DEGREE * (error * dynamic_gain2 + ((error - last_pil_diff) * PILLAR_PD2))))#*(normalized_y+1))#*(normalized_y/2+0.5))
        else:
            angle = -(int((error * dynamic_gain) + ((error - last_pil_diff) * PILLAR_PD)))#*(normalized_y+1))#*(normalized_y/2+0.5)))
        
        
        if (cX  < 100 or cX > 540 ) and cY > 310:
